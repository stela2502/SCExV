	 ...
	 */
	 ...
	 this.setprMatrix = function(id) {
		 ...
		 ranges = [(bbox[1]-bbox[0])*scale[0]/2,
		           ...
		           radius = Math.sqrt(this.sumsq(ranges))*1.1; // A bit bigger to
		 // handle labels
...
//			 Vertex Av = normal.getRecycled(elem);
	 ...
	 /*
	  * this is only used by .writeWebGL; rglwidget has no debug element and
	  * does the drawing in rglwidget.js
	  */
	 ...
	 this.drawSceneSave = function() {
		 this.drawScene();
		 lastImage = new Image(); // global var
		 lastImage.src = this.canvas.toDataURL('image/png');
	 };

...
(function() {
	...
	/*
	 * set element of 1d or 2d array as if it was flattened. Column major, zero
	 * based!
	 */
	...
	this.setprMatrix = function(id) {
		...
		ranges = [(bbox[1]-bbox[0])*scale[0]/2,
		          ...
		          radius = Math.sqrt(this.sumsq(ranges))*1.1; // A bit bigger
																// to handle
																// labels
...
if (nhits >= 3) {
	...
	/*
	 * this is only used by .writeWebGL; rglwidget has no debug element and does
	 * the drawing in rglwidget.js
	 */
	...
	this.initGL0 = function() {
		if (!window.WebGLRenderingContext){
			this.debug("Your browser does not support WebGL. See http://get.webgl.org", this.getSnapshot());
			return;
		}
		try {
			this.initGL();
		...
		if ( !this.gl ) {
			this.debug("Your browser appears to support WebGL, but did not create a WebGL context.  See http://get.webgl.org",
					this.getSnapshot());
			return;
		}
	};
...
rgltimerClass = function(Tick, startTime, interval, stopTime, value, rate, loop, actions) {
	...
	this.multiplier = 1;                /*
										 * multiplier for fast-forward or
										 * reverse
										 */
